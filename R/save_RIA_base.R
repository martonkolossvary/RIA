  ...
  for (i in 1: length(names_stat)) {
    if(RIA_name == "stat_geometry") {
      ...
      for (j in 1: length(names_stat_geo)) {
        name_data_geo <- t(RIA:::list_to_df(eval(parse(text = paste0("RIA_image$",RIA_name, "$", names_stat[i], "$", names_stat_geo[j])))))
    ...
    name_data <- t(RIA:::list_to_df(eval(parse(text = paste0("RIA_image$",RIA_name, "$", names_stat[i])))))
